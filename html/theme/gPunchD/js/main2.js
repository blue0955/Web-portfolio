...
//     let wrapper = document.querySelector("[data-lenis-prevent]");
//     if (wrapper) {
//       lenisWrapper = new Lenis({
//         wrapper: wrapper,
//         content: wrapper,
//       });
//     }
...
//     function raf(time) {
//       lenis.raf(time);
//       if (lenisWrapper) lenisWrapper.raf(time);
...
function handleVideos() {
  ...
  if (projectContent) {
    const iframes = projectContent.querySelectorAll("iframe");
    if (iframes.length > 0) {
      iframes.forEach((iframe) => {
        const parent = iframe.parentElement;
        const player = new Vimeo.Player(iframe);

        if (iframe.hasAttribute("squared")) {
          parent.setAttribute("squared", "");
        }
        ...
        async enter(data) {
          if (browserNav) {
            ...
            flip(
              ...
              data.next.container.querySelector(".ratio-project-header")
            );
            return gsap
...
function handleSnippetsFilter() {
  let e = document.querySelector("[snippets-filter-list]");
  if (!e) return;
  let t = document.querySelectorAll(".snippets-category-tag"),
    r = e.querySelectorAll(".snippets-coll-item");
  t.forEach((e) => {
    let t = (function e(t, r) {
      let n = 0;
      for (let o = 0; o < t.length; o++) t[o].dataset.category === r && n++;
      return n;
    })(r, e.querySelector("[category-count]").getAttribute("category-count"));
    e.querySelector("[category-count]").textContent = `${t < 10 ? "0" + t : t}`;
  });
  let n = !1,
    o = {
      opacity: 1,
  ...
  t.forEach((l) => {
    l.addEventListener("click", function (l) {
      if ((l.preventDefault(), this.classList.contains("on") || n)) return;
      t.forEach((e) => e.classList.remove("on")), this.classList.add("on");
      let i =
        this.querySelector("[category-count]").getAttribute("category-count");
      !(function t(l) {
        if (n) return;
        n = !0;
        let i = Array.from(r).filter((e) => {
          let t = Array.from(e.querySelectorAll(".snippets-category"));
          return t.some((e) => e.textContent.trim() === l.trim());
        });
        gsap.to(r, {
          ...a,
          onComplete() {
...
function loadWebgl() {
  let pageContainer = document.querySelector("[load-webgl-simulation]");
  if (!pageContainer) return;
  let script = document.createElement("script");
  script.src = pageContainer.getAttribute("load-webgl-simulation");
  pageContainer.appendChild(script);
}
function handleGlobalCode() {
  // Count total Works
  let totalWorkItems = document.querySelectorAll(
    "[count-work-coll-list] > div"
  ).length;
  if (!totalWorkItems == 0) {
    document.querySelectorAll("[count-total-work]").forEach((ele) => {
      ele.innerHTML = totalWorkItems;
    });
  }

  handleDonutBtn();
  function handleDonutBtn() {
    const donutBtn = document.getElementById("donut-btn");
    if (!donutBtn) return;
    const smallDonutIcons = donutBtn.querySelectorAll(".small-donut");

    // Store initial positions of smallDonutIcons
    const initialPositions = [];
    smallDonutIcons.forEach((icon) => {
    ...
    // Animation for hovering in
    function animateIn() {
      smallDonutIcons.forEach((icon) => {
        const randomTop = (Math.random() - 0.5) * donutBtn.offsetHeight * 3.5;
        const randomLeft = (Math.random() - 0.5) * donutBtn.offsetWidth * 2;
        let scale = Math.random() * (1.3 - 0.7) + 0.7;
        // icon.style.opacity=0
        icon.setAttribute("scale", scale);
        donutBtn.querySelector(".donut-btn-inner").style.zIndex = 1;
        let left, top;
        gsap.to(icon, {
          duration: 0.7,
          // opacity: 1,
    ...
    // Animation for hovering out
    function animateOut() {
      donutBtn.querySelector(".donut-btn-inner").style.zIndex = -1;
      smallDonutIcons.forEach((icon, index) => {
        gsap.to(icon, {
          duration: 0.5,
          scale: 1,
          // opacity: 0,
    ...
    donutBtn.addEventListener("mouseenter", animateIn);
    donutBtn.addEventListener("mouseleave", animateOut);
  }
}
function handleDonuts() {
  let wrapper = document.querySelector(".donuts-wrapper");
  if (!wrapper) return;
  let donutImages = wrapper.querySelectorAll(".donut");
  let numDonutsToAdd = Math.floor(Math.random() * (15 - 16 + 1)) + 16;

  let donutSize = 100;
  let minDistance = 20;

  for (let i = 0; i < numDonutsToAdd; i++) {
    let donutClone =
      donutImages[Math.floor(Math.random() * donutImages.length)].cloneNode(
        true
      );
    let scale = Math.random() * (1.3 - 0.7) + 0.7;
    donutClone.classList.remove("hide");
    donutClone.setAttribute("scale", scale);
    let left, top;

    do {
      left = Math.random() * (wrapper.offsetWidth - donutSize * scale);
      top = Math.random() * (wrapper.offsetHeight - donutSize * scale);
    } while (isColliding(left, top, donutSize * scale));

    // donutClone.style.transform = `scale(${scale})`;
    donutClone.style.left = `${left}px`;
    donutClone.style.top = `${top}px`;

    wrapper.appendChild(donutClone);
    ...
    tl.to(donutClone, {
      ...
      {
        delay: 0.5,
        scale: donutClone.getAttribute("scale"),
        duration: Math.random() * (2 - 1.5) + 1.5,
        // Random duration between 1.5 and 2 seconds
        y: wrapper.offsetHeight + 200,
        // Move down below the screen
        ease: "power1.in",
        onComplete: () => {
          gsap.to(donutClone, {
            duration: 0.25,
        ...
        // Remove the donut after animation completes
      },
      "<"
    );
  }
  ...
  function isColliding(x, y, size) {
    for (const donut of wrapper.querySelectorAll(".donut")) {
      const rect = donut.getBoundingClientRect();
      if (
        x < rect.right + minDistance &&
        x + size > rect.left - minDistance &&
        y < rect.bottom + minDistance &&
        y + size > rect.top - minDistance
      ) {
        return true;
      }
    }
