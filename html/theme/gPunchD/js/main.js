...
function handleFlickity() {
  ...
  function destroyFlickity() {
    flickities.forEach((flkty) => flkty.destroy());
    flickities = [];
  }

  function updateFlickity() {
    const flickityElems = document.querySelectorAll("[data-carousel]");
    const isSmallScreen = window.matchMedia("(max-width: 479px)").matches;

    if (isSmallScreen) {
      if (flickities.length === 0) {
        initFlickity(flickityElems);
      }
    } else {
      if (flickities.length > 0) {
        destroyFlickity();
...
function handleScrollSmooth() {
  ...
  const lenis = new Lenis({
    ...
    smoothTouch: false, // 터치 디바이스에서도 부드러운 스크롤 활성화
...
const sections = [
  ...
  { selector: ".section-contact", heightRatio: 0.35 } // 높이를 조금 더 늘려봅니다.
];

// 스크롤과 리사이즈 이벤트 리스너
function handleScrollProgress() {
  const scrollY = window.scrollY;
  const maxScroll = document.body.scrollHeight - window.innerHeight; // 전체 문서의 높이 - 뷰포트의 높이

  let currentStart = 0;

  sections.forEach(({ selector, heightRatio }, index) => {
    const element = document.querySelector(selector);
    if (!element) return; // 섹션이 없으면 건너뛰기

    // 각 섹션의 실제 높이를 계산
    const sectionHeight = heightRatio * maxScroll;

    // 특정 섹션에 오프셋 추가
    const offset = selector === ".section-contact" ? 200 : 0; // .section-contact의 시작점 지연
    ...
    progress = Math.max(0, Math.min(1, progress)); // progress를 0과 1 사이로 클램핑
...
// 섹션의 최소 높이를 설정하는 함수
function adjustSectionContactMinHeight() {
  const sectionContact = document.querySelector(".section-contact");
  if (!sectionContact) return;

  const viewportHeight = window.innerHeight;
  // 모바일 화면일 때 최소 높이를 설정 (예: 1500px)
  if (viewportHeight < 768) {
    sectionContact.style.minHeight = `${Math.max(850, viewportHeight)}px`;
  }
}

// 리사이즈 이벤트 시 실행
window.addEventListener("resize", adjustSectionContactMinHeight);

// 페이지 로딩 시에도 실행
adjustSectionContactMinHeight();
...
const customCursorFull = document.querySelector(".cursor-wrap .cursor");
const customCursorDot = document.querySelector(".cursor-wrap i");

$("body a,body button").hover(
  function () {
    gsap.to(customCursorFull, 0.1, { scale: 0.3 });
    gsap.to(customCursorDot, 0.1, { opacity: 1 });
  },
  function () {
    gsap.to(customCursorFull, 0.1, { scale: 1 });
    gsap.to(customCursorDot, 0.1, { opacity: 0 });
  }
);


document.addEventListener("DOMContentLoaded", handleCursor2);
...
function handlePage() {
  ...
  const pageName = page ? page.getAttribute("data-page").toLowerCase() : "home";
  const footer = document.querySelector(".footer");

  // Home 페이지 처리 (Contact는 제외)
  if (pageName === "home") {
...
function updateCurrentPageClass() {
  const links = document.querySelectorAll(".nav-content a"); // 모든 네비게이션 링크 선택
  const currentPage = document
    .querySelector("[data-page]")
    ?.getAttribute("data-page"); // 현재 페이지의 data-page 값

  links.forEach((link) => {
    let href = link.getAttribute("href")?.trim() || ""; // 링크의 href 속성 값
    if (!href) return; // href가 비어 있으면 스킵

    let hrefPath = "";
    if (href === "/") {
      hrefPath = "home"; // "/" (홈)인 경우 "home"으로 설정
    } else {
      let parts = href.split("/"); // URL을 '/' 기준으로 나누기
      if (parts.length > 2 && !isNaN(parts[parts.length - 1])) {
        // contact/번호 같은 URL 패턴인지 확인
        hrefPath = parts[parts.length - 2]; // 마지막 값(번호) 대신 그 앞의 값(contact)을 사용
      } else {
        hrefPath = parts.pop().replace(".php", "").toLowerCase(); // 기존 방식 유지
      }
    }

    const pageName = currentPage ? currentPage.toLowerCase() : ""; // 현재 페이지 data-page 값

    // 디버깅용 로그
    // console.log("Current Page (data-page):", pageName);
    // console.log("Link HREF (processed):", hrefPath);

    // 현재 페이지와 링크의 파일명이 일치하면 aria-current와 클래스 추가
    if (pageName && pageName === hrefPath) {
...
// DOMContentLoaded 이벤트로 실행
document.addEventListener("DOMContentLoaded", updateCurrentPageClass);
...
function handleBarba() {
  const isPopup = window.location.pathname.includes("/bbs/"); // 팝업 URL 감지
  if (isPopup) {
    console.log("Popup detected: Transitions disabled.");
    return; // 팝업 창에서는 트랜지션 비활성화
  }
