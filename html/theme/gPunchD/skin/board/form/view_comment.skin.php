...
<section id="bo_vc">
    ...
    <div id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?> class="commentBlock">
		<?php if ($cmt_depth) { ?><i class="fa fa-reply  cmt_depth"></i> <?php } ?>
		<div class="pf_img"><?php if(get_member_profile_img($list[$i]['mb_id']) == '<img src="'.G5_IMG_URL.'/no_profile.gif" alt="profile_image">') { echo '<img src="'.G5_THEME_IMG_URL.'/no_profile.gif" alt="profile_image">';} else { echo get_member_profile_img($list[$i]['mb_id']);} ?></div>
        <div style="z-index:<?php echo $cmt_sv; ?>" class="cmt_header">
        ...
        <div class="cmt_contents">
            ...
            <ul class="bo_vc_act">
                ...
                <?php if ($list[$i]['is_del'])  { ?><li><a href="<?php echo $list[$i]['del_link'];  ?>" onclick="return comment_delete();" class="btn_b03 btn_trash"><i class="fa fa-trash"></i><span class="sound_only">삭제</span></a></li><?php } ?>
            </ul>
            <?php } ?>
        </div>
    ...
    <?php } ?>
    <?php if ($i == 0) { //댓글이 없다면 ?><p id="bo_vc_empty"><i class="bo_vc_emptyIco fa fa-exclamation-triangle"></i> <br><br>등록된 댓글이 없습니다.</p><?php } ?>
...
<div id="bo_vc_w" class="bo_vc_w">
    ...
    <?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?>><?php echo $c_wr_content; ?></textarea></div>
