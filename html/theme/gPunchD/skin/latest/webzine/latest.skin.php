    ...
    <?php for ($i=0; $i<count($list); $i++) { 
		...
		if(isset($thumb['src']) && $thumb['src']) {
			...
			if(isset($youtubeID1[2]) && $youtubeID1[2] == "youtu.be") {
				$img_content = '<i class="webzLatThumb" style="background-image:URL(//img.youtube.com/vi/'.$youtubeID1[3].'/maxresdefault.jpg);"><img src="'.G5_THEME_URL.'/plugin/websiting/imageSpacer.php?w='.$thumb_width.'&h='.$thumb_height.'" class="youtubeImbBlank" alt=""></i>';
				...
				if(isset($youtubeID1[2]) && $youtubeID1[3] == "shorts" || $youtubeID1[3] == "live") {
					...
					$img_content = '<i class="webzLatThumb" style="background-image:URL(//img.youtube.com/vi/'.$youtubeIDnew[$i][0].'/maxresdefault.jpg);"><img src="'.G5_THEME_URL.'/plugin/websiting/imageSpacer.php?w='.$thumb_width.'&h='.$thumb_height.'" class="youtubeImbBlank" alt=""></i>';
				} else {
					$youtubeIDnew[$i] = explode('=',$youtubeID1[3]); 
					$youtubeIDnewID[$i] = explode('&',$youtubeIDnew[$i][1]); 
					$img_content = '<i class="webzLatThumb" style="background-image:URL(//img.youtube.com/vi/'.$youtubeIDnewID[$i][0].'/maxresdefault.jpg);"><img src="'.G5_THEME_URL.'/plugin/websiting/imageSpacer.php?w='.$thumb_width.'&h='.$thumb_height.'" class="youtubeImbBlank" alt=""></i>';				
				}
			} else {
				$img_content = "<!-- 이미지 없음 -->";
			}
