...
if ($board['bo_use_category']) {
    ...
    if ($bo_table == "works") {
        ...
        for ($i = 0; $i < count($categories); $i++) {
            ...
            // 중복 카테고리를 각각 <span> 태그로 출력
            $categories_split = explode(',', $category); // 카테고리를 ','로 분리
            $formatted_categories = '';
            foreach ($categories_split as $cat) {
                $cat = trim($cat); // 공백 제거
                if ($cat != '') {
                    $formatted_categories .= '<span class="category-item">' . htmlspecialchars($cat) . '</span>'; // 각 카테고리에 span 추가
                }
            }

            // <a> 태그 안에 카테고리 메시지와 분리된 카테고리 삽입
            $category_option .= '>' . $category_msg . $formatted_categories . '</a></li>';
        }
    } else {
        /* original */
        for ($i=0; $i<count($categories); $i++) {
