...
// 2025-01-24 추가 시작
$wr_homepage = isset($_POST['wr_homepage']) ? $_POST['wr_homepage'] : '';
// 2025-01-24 추가 끝
...
if ($bo_table !== "works") {  //추가
    ...
    // 28-01-14 수정 start
     if (isset($_FILES['visual_file']['tmp_name']) && $_FILES['visual_file']['tmp_name']) {
          $wr_11 = isset($write['wr_11']) ? $write['wr_11'] : '';
        }
    } else {
        $wr_11 = '';
    // 28-01-14 수정 end
...
// 2025-01-25 수정
// TIMELINE 값 저장
if (isset($_POST['wr_timeline'])) {
    $wr_timeline = sql_real_escape_string($_POST['wr_timeline']);
    sql_query("UPDATE {$write_table} SET wr_timeline = '{$wr_timeline}' WHERE wr_id = '{$wr_id}'");
}

// OVERVIEW 값 저장
if (isset($_POST['wr_overview'])) {
    $wr_overview = sql_real_escape_string($_POST['wr_overview']);
    sql_query("UPDATE {$write_table} SET wr_overview = '{$wr_overview}' WHERE wr_id = '{$wr_id}'");
}
// 2025-01-25 수정 끝
$wr_content = '';
...
if (isset($_FILES['visual_file']['tmp_name']) && $_FILES['visual_file']['tmp_name']) {
    $upload_dir = G5_DATA_PATH . '/visual_image';
    @mkdir($upload_dir, G5_DIR_PERMISSION);
    @chmod($upload_dir, G5_DIR_PERMISSION);

    $tmp_file = $_FILES['visual_file']['tmp_name'];
    $filename = $_FILES['visual_file']['name'];
    $filepath = $upload_dir . '/' . $filename;

    if (move_uploaded_file($tmp_file, $filepath)) {
        $file_url = G5_DATA_URL . '/visual_image/' . $filename;
        sql_query("UPDATE {$write_table} SET wr_11 = '$file_url' WHERE wr_id = '$wr_id'");
    }
}
//25-01-14 수정 end
    ...
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (is_uploaded_file($tmp_file)) {
            ...
            if (!$is_admin && $filesize > $board['bo_upload_size']) {
             ...
             // 비주얼 이미지 처리 (bf_file[10]에 해당)
            if ($i == 10) {
                $visual_dir = G5_DATA_PATH . '/visual_image';
                @mkdir($visual_dir, G5_DIR_PERMISSION);
                @chmod($visual_dir, G5_DIR_PERMISSION);

                $visual_filepath = $visual_dir . '/' . $filename;

                // 기존 wr_11 파일 삭제
                if (!empty($write['wr_11'])) {
                    $existing_file = str_replace(G5_DATA_URL, G5_DATA_PATH, $write['wr_11']);
                    if (file_exists($existing_file)) {
                        @unlink($existing_file);
                    }
                }

                if (move_uploaded_file($tmp_file, $visual_filepath)) {
                    $visual_url = G5_DATA_URL . '/visual_image/' . $filename;

                    // wr_11에 URL 저장
                    sql_query("UPDATE {$write_table} SET wr_11 = '$visual_url' WHERE wr_id = '$wr_id'");
                } else {
                    $file_upload_msg .= '비주얼 이미지를 업로드하지 못했습니다.\\n';
                }
            }
...
// 비주얼 이미지 삭제 처리
if (isset($_POST['visual_file_del']) && $_POST['visual_file_del'] == '1') {
    if (!empty($write['wr_11'])) {
        $existing_file = str_replace(G5_DATA_URL, G5_DATA_PATH, $write['wr_11']); // 실제 파일 경로 변환
        if (file_exists($existing_file)) {
            @unlink($existing_file); // 실제 파일 삭제
        }
    }
    // DB에서 wr_11 필드 초기화
    sql_query("UPDATE {$write_table} SET wr_11 = '' WHERE wr_id = '{$wr_id}'");
}
